...
3. Your Application will make a call to the Concur Authorization service to trigger the OTP email using [OTP grant](/api-reference/authentication/apidoc.html#otp_grant) while being Geo Aware.
...
To [revoke](/api-reference/authentication/apidoc.html#revoke_token) a user&#39;s refresh\_token call the connections endpoint with a DELETE action while being Geo Aware.
