...
Before you can obtain a JSON Web Token (JWT), you need to register an application with Concur. You can do this by contacting your Partner Enablement Manager or Partner Account Manager. Once you have registered an application, you will receive a `clientId` and `clientSecret`. The `clientId` is a unique UUID4 identifier for your application, and the `clientSecret` is your application's password. You will be using this client to obtain tokens either for the client itself, or on behalf of a user. 
...
The `password` grant flow is used when you need to authenticate a user, using its `username` and `password`. Note, the user you're requesting an `accessToken` for needs to be in the same environment as your client. Typically this is used by apps that have a login screen where the user's credentials are captured or stored. 
...
Full docs: --LINK TO PASSWORD GRANT--
...
Armed with the `accesstoken` you can start making calls to Concur's API. Here's an example to retrieve profile information for a user in the Production environment using cURL ([Base URis for other Environments](--LINK TO BASE URI--)):
...
Full docs: --LINK TO LOOKING UP USER--
...
FULL DOCS: --LINK TO REFRESH GRANT--
