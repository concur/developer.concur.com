...
import { appChangeRequest, appChangeFailure, appChangeSuccess, generateAppSecret, postApp, putApp } from '../../actions/appChange';
...
describe('putApp', () => {
  ...
  it('creates a appChangeSuccess action when fetching is successful', () => {
    ...
    return store.dispatch(putApp(app))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
  ...
  it('creates a appChangeFailure action when fetching fails', () => {
    ...
    const expectedActions = [
      appChangeRequest(),
      appChangeFailure(`request to ${process.env.DEVCENTER_API_FORMS}/applications/${app.id} failed, reason: Server is down`),
    ];
