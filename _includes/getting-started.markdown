...
In order for an application to call a Concur API, you need to obtain an `accessToken` on behalf of either a User, Company or Application. There are multiple ways of obtaining an `accessToken` through the various grants ([Password](#password-grant), [Authorization](#authorization-grant), [Client Credentials](#client-credentials-grant), [One-time Password](#one-time-password-grant)) . Here is the simplest example how you would obtain an `accessToken` for a User by using the Password grant.
...
curl -X POST \
-H 'concur-correlationid: nameofapp' \
--data 'username=$username&password=$password&grant_type=password&client_secret=$client_secret&client_id=$client_id'
"$oauth2_base/v0/token"
```

## Calling an API with the accessToken
...
Armed with the `accessToken` you can start making calls to Concur's API. Here's an example to retrieve profile information for a User in the Production environment using cURL ([Base URIs for other Environments](#base-uris)):
...
```shell
curl -X POST \
--data 'client_id=$client_id&client_secret=$client_secret&grant_type=refresh&refresh_token=<old refresh token>' \
'https://us.api.concursolutions.com/oauth2/v0/token'
```

[FULL DOCS](#refreshing-a-token)
