...
* **Request parameters**
  ...
  | Parameter | Required/Optional | Description |
  |-----------|-----------|---------------------|
  | `requests/{requestKey}` | required | The `requests` keyword and the identifier for the desired travel request. <br><br> **URI Source**: The URI is provided in the **ObjectURI** element of the [Event Notification](/api-reference/callouts/event-notification.html) request. |
