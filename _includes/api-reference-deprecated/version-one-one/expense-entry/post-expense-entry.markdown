...
* **Query parameters**
  ...
  |-----------|-----------|---------------------|
  | {reportId} | required | The unique identifier for the expense report and the entry keyword. <br><br> **URI Source:** The reportId value is returned in the `ReportID` element by the Get List of Reports function, and as part of the `Report-Details-Url` element of the Post Expense Report Header function. |
  | {entryId} | optional | The unique identifier for the expense entry. <br><br> **URI Source:** The entryId value is returned in the `RpeKey` element of the Get Full Report Details v1.1 function, and as part of the `Report-Entry-Details-Url` value returned by this function. |

* **Headers**

  | Name | Description |
  | ---- | ----------- |
  | `Authorization` | Authorization header with OAuth token for valid Concur user. Required. <br><br> The OAuth Consumer must have one of the following roles to post entries for reports that they do not own: Web Services Admin for Professional, or Can Administer for Standard. |
