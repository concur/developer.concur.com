...
In order for an application to call a Concur API, you need to obtain an `accessToken` on behalf of either a User, Company or Application. There are multiple ways of obtaining an `accessToken` through the various grants ([Password](https://developer.concur.com/api-preview/auth/apidoc.html#password_grant), [Authorization](https://developer.concur.com/api-preview/auth/apidoc.html#auth_grant), [Client Credentials](https://developer.concur.com/api-preview/auth/apidoc.html#client_credentials), [One-time Password](https://developer.concur.com/api-preview/auth/apidoc.html#otp_grant)) . Here is the simplest example how you would obtain an `accessToken` for a User by using the Password grant.
