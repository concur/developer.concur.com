...
This document covers itinerary related events on the public topic `public.concur.travel.itinerary` in the Event Subscription Service (ESS).
...
---|---|---|---
`id`|`UUID`|RFC 4122|**Required** EventId, unique identifier of this event.
...
`userId`|`UUID`|RFC 4122|**Required** User Unique identifier from profile service that that this itinerary belong to.
`hrefs`|`map`|-|**Omitempty, Required** Key-value pairs providing Itineray schema version and URI of the itinerary record retrieved via the Itinerary v4 API.
