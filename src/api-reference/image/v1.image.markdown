...
`expenseentry/{entryID}`|`string`|-|The {keyword}/{identifier} for the desired expense entry. The entryId is returned in the `RpeKey` within `ExpenseEntry` portion of the response from the [Get Report Details v1.1](/api-reference-deprecated/version-one-one/expense-report/get-report-full-details.html) API.
