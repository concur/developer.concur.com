...
So please pass empty array `[]` for those fields.
...
`existing`| `string`|-|**READ ONLY** Indicates if the user is a new, existing or removed team member of the budget. `NEW` means the user was newly added as a reportee of the budget owner. `Existing` means already a reportee and `REMOVED` means the team member is not a direct reportee of the budget owner. 
