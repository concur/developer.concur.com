...
`budgetItemHeaders`|`array`|[`budgetItemHeader`](#budgetItemHeader)|List of budget items.
...
`budgetAmounts`|`array`|[`budgetAmounts`](#budgetAmounts)|The accumulated budget amounts for this budget.  **READ ONLY**
`budgetManagers`|`array`|[`budgetPerson`](#budgetPerson)|Manager Hierarchy only.
...
`budgetType`|`string`|-| The budget type indicates if the budget item is personal or not = ['PERSONAL_USE', 'BUDGET', 'RESTRICTED']
...
`createdDate`|`date`|YYYY-MM-DD|The time the budget item header was created. Field is read only. Date in UTC.
...
`dateRange`|`dateRange`|-| Specify custom date range for budget.**READ ONLY**
...
`lastModifiedDate`|`dateRange`|-|  The last time the budget item header was updated. Field is read only. Date in UTC.
...
`budgetAmounts`|`array`|[`budgetAmounts`](#budgetAmounts)|The accumulated budget numbers for this budget.  **READ ONLY**
`budgetItemBalances`|`array`|[`budgetItemBalance`](#budgetItemBalance)|Shows the break-out of budget spending by product and workflow state.  **READ ONLY**
...
`budgetName`|`string`|-|The admin-facing name of this budget.**READ ONLY**
`budgetItemHeaderId`|`string`|-|The unique id of the header that contains this budget item detail.**READ ONLY**
...
`detailDashboardBudgetItemAdjustmentDTOs`|`array`|[`detailDashboardBudgetItemAdjustment`](#detailDashboardBudgetItemAdjustment) | List of budget adjustments.**READ ONLY**
`id`|`string`|-|The key for this object.
`budgetItemHeaderId`|`string`|-|The unique id of the header that contains this budget item detail.
...
Name|Type|Format|Description
---|---|---|---
`adjustedBudgetAmount`|`decimal`|-|The amount adjusted against this budget.  **READ ONLY**
...
`threshold`|`string`|-|The indicator of whether this budget is under the alert limit, equal to or over the alert limit but under the control limit, or equal to or over the control limit. Supported values: `UNDER`, `ALERT`, `OVER`  **READ ONLY**
`unExpensedAmount`|`decimal`|-|The amount of unexpensed items like travel bookings, quick expenses, or e-receipts **READ ONLY**
`unExpensedSettings`|`string`|-|The company's budget setting for unexpensed items. Applies to all budgets for the company. Supported values: `SHOW_UNSUBMITTED_EXPENSES_AS_PENDING`, `SHOW_UNSUBMITTED_EXPENSES_BALANCE`, `DO_NOT_SHOW_UNSUBMITTED_EXPENSES`. **READ ONLY**
...
`employeeId`|`string`|-|The employee ID of the person to lookup.**READ ONLY**
`employeeUuid`|`string`|-|The unique identifier for this user.  This must match the UUID from Concur's profile service.
...
`name`|`string`|-|The name for this expense type if it maps to an expense type set up in your SAP Concur product(s). **READ ONLY**
`id`|`string`|-|The budget service's key for this object.
...
`operator`|`string`|-|The comparison to use when matching values for this tracking field.  Supported values: `EQUAL`, `INLIST`, `ISBLANK`, `NOTEQUAL`, `NOTINLIST`, `ISNOTBLANK`, `ISTRUE`, `ISFALSE`, `ISNOTTRUE`, `ISNOTFALSE`.
...
`featureTypeCode`|`string`|-|The product type for this balance. Supported values: `REQUEST`, `TRAVEL`, `EXPENSE`, `PAYMENT_REQUEST`.  **READ ONLY**
`featureTypeSubCode`|`string`|-|The feature type sub code for this balance. = ['QUICK_EXPENSE', 'ERECEIPT', 'CREDIT_CARD', 'PERSONAL_CARD', 'TRIP', 'RECEIPT_CAPTURE', 'BILLING_STATEMENT', 'MANUAL', 'NONE', 'BUDGET_AMOUNT', 'SPENT_AMOUNT', 'PENDING_AMOUNT', 'PRE_AUTHORIZED', 'PURGE_ADJUSTMENT'] **READ ONLY**
`workflowState`|`string`|-|Supported values: `UNSUBMITTED`, `UNSUBMITTED_HELD`, `SUBMITTED`, `APPROVED`, `PROCESSED`, `PAID`. **READ ONLY**
`id`|`string`|-|The budget service's key for this object.
...
Name|Type|Format|Description
---|---|---|---
`currentYear`|`boolean`|-|`True` if this is the current fiscal year based on the current date and time, `False` otherwise. **READ ONLY**
...
`displayName` | `string`| - | Display name for fiscal year. For date range budget item we use this field to display.**READ ONLY**
`monthlyFiscalPeriods`	|	`array`	|	[`fiscalPeriod`](#fiscalPeriod)	|	 The list of monthly Fiscal Periods in this Fiscal Year. Fiscal periods must complete fill the parent Fiscal Year with no overlaps.**READ ONLY**
`quarterlyFiscalPeriods`	|	`array`	|	[`fiscalPeriod`](#fiscalPeriod)	|	 The list of quarterly Fiscal Periods in this Fiscal Year.  If this parameter is not specified, quarterly Fiscal Periods are automatically generated based on the monthly Fiscal Periods supplied.**READ ONLY**
`yearlyFiscalPeriods`	|	`array`	|	[`fiscalPeriod`](#fiscalPeriod)	|	 The list of yearly Fiscal Periods in this Fiscal Year.  If this parameter is not specified, one period is created that fills the Fiscal Year.**READ ONLY**
`customFiscalPeriods`	|	`array`	|	[`fiscalPeriod`](#fiscalPeriod)	|	 The list of custom Fiscal Periods in this Fiscal Year.  Custom Fiscal Periods are API-only and will not display on user budget dashboards.**READ ONLY**
...
`fiscalPeriods`	|	`array`	|	[`fiscalPeriod`](#fiscalPeriod)	|	 The list of all Fiscal Periods in this Fiscal Year. **READ ONLY**
`displayName` | `string`| - | Display name for fiscal year. For date range budget item we use this field to display.**READ ONLY**
...
Name|Type|Format|Description
---|---|---|---
`currentPeriod`|`boolean`|-|`True` if this is the current fiscal period based on the current time, `False` otherwise. **READ ONLY**
...
Name|Type|Format|Description
---|---|---|---
`adjustmentType`	|	`string`	|	-	|	The type of adjustment being made. Only a user reference field = ['BUDGET_BALANCE', 'FUND_TRANSFER', 'EXPENSE', 'PAYMENT_REQUEST', 'PURCHASE_REQUEST', 'REQUEST', 'PURGE_ADJUSTMENT']
`amount`	|	`decimal`	|	-	|	The value of the amount adjusted (+/-)
`amountType`	|	`string`	|	-	|	The amount type of the field = ['QUICK_EXPENSE', 'ERECEIPT', 'CREDIT_CARD', 'PERSONAL_CARD', 'TRIP', 'RECEIPT_CAPTURE', 'BILLING_STATEMENT', 'MANUAL', 'NONE', 'BUDGET_AMOUNT', 'SPENT_AMOUNT', 'PENDING_AMOUNT', 'PRE_AUTHORIZED', 'PURGE_ADJUSTMENT']
`description`	|	`string`	|	-	|	The short description of the adjustment.
`transactionDate`	|	`date`	|	YYYY-MM-DD	|	The transaction date of adjusted spend. Required when the adjustment is made for amount type SPENT_AMOUNT or PENDING_AMOUNT.
