...
Retrieve all budget items in groups of up to 50 items.  Due to response size and performance considerations, this endpoint does not return budgetItemDetails.  Use the [GET All Budget Items](#getall) to retrieve all for the details for a single budget.
...
`costObjects`|`array`|[`costObjectValue`](#costObjectValue)|The cost object list for matching spending items.
...
`fiscalPeriod`|`array`|[`fiscalPeriod`](#fiscalPeriod)|**Required** The fiscal period for this budget amount.  Only the ID of the fiscal period, which can be retrieved from the [Fiscal Year service](#/api-reference/budget/v4.fiscal-year.html), is required for creating/updating a budget.
`id`|`string`|-|The key for this object.
...
Name|Type|Format|Description
---|---|---|---
`featureTypeCode`|`string`|-|**Required** The product that this expense type applies to--Purchase Request, Payment Request (Invoice), Expense, or Request. Supported values: `PURCHASE_REQUEST`, `PAYMENT_REQUEST`, `EXPENSE`, `REQUEST`
