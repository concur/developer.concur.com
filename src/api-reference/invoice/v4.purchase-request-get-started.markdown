...
Many Concur Invoice clients have external systems and catalogs that have part or service lists with pricing that they use for ordering products or services for their business needs. If they use SAP Concur pre-authorization using purchase request, this data must be entered manually into the purchase request cart and submitted for approval by the requesting employee. Using this API and a client-built integration, the requestor can remain in the external system and send these details to SAP Concur. This automates and streamlines the purchase request creation and submission process. The requestor can browse and select the items from the external system with quantities and other details needed, and then trigger it to be sent to SAP Concur. A purchase request will be created via the API, and the purchase request will be submitted into the workflow. The API returns a response message with a record identifier (URI), which can be used to get the basic details of the created purchase request: Concur Purchase Request number, workflow status, exceptions, and once approved, the resulting SAP Concur purchase order number. This Get Purchase Request Details method can be called at any point after creation to return the real time status of that purchase request as it goes through pre-authorization in SAP Concur.
