...
`policyExternalId`|`string`|-|The external identifier of the policy that should be associated with the purchase order. If not supplied, the API will use the default policy set up for the user group assigned to the requesting employee. This is the **external Id** from the Invoice Policy configuration. Clients will need to get these Ids from their SAP Concur contact if they need to assign policies other than the group default.
`currencyCode`|`string`|-|**Required**: The 3-letter ISO 4217 currency code of the currency that is associated with the purchase order. This code will be used for all items on this request. Example: USD
`notesToSupplier`|`string`|-|Notes to print on the transmitted purchase order PDF sent to your supplier.
...
`billToAddressCode`|`string`|-|The billing address of the Purchase Request to be used for Invoicing. The accepted value is the address code from BillTo record. If not supplied the API will use the policy's default BillTo address.
...
`url`|`array`|-|A URL related to the item. You can have multiple URLs per item.
...
Name|Type|Format|Description
---|---|---|---
`errorCode`|`string`|-|An error code indicating why a field had failed validation.
...
listCodeNotfound|Cannot find the supplied List code.
