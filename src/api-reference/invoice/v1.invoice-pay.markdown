...
```json
{
  ...
  "paymentInitiationDate" : "2018-05-08",
...
Name|Type|Format|Description
---|---|---|---
`providerReference`|`string` (max 100)|-|Unique identifier of the payment in the pay provider's system
...
`statusMessage`|`string` (max 255)|-|description of the status *Partner can provide anything they want*
...
`thirdPartyPaymentIdentifier`|`string` max (255)|-|Check number if the payment was done via check or trace number for ACH payments
`paymentMethod`|`string`|[`Payment Method`](#schema-payment-method)|**Required if the status is Paid, Check Processed, Card Processed** Payment method used by the pay provider
