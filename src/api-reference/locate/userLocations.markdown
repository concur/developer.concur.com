...
### Transaction
Itineraries can be added or cancelled. This section allows you to indicate whether this is an addition of a new itinerary or cancellation of an existing itinerary.A Cancel request for this scenario is a partial cancellation or update to an existing transaction so they simply need to resend that transaction request without that cancelled segment and our service removes the previous transaction details completely and saves the latest transaction details
