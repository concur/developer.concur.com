...
For the purposes of ERP integration this API is available to Professional Edition.

Both Edition types produce extract files that you will use as part of the integration, however, there are distinct differences between them in obtaining the extract file, so it is important to first determine the Edition Type.
...
1. [Get an array of extract definitions](#get-definitions)
  1. Locate the desired definitions for your client. You may need to obtain additional details as there may be multiple files to obtain depending on client requirements.
  1. Record the Definition ID to use in subsequent API requests.
1. [Get an array of extract jobs](#get-job-array)
  1. OPTIONAL: the Partner only needs to do this if the Partner is running the job, otherwise skip this step.
  1. Determine which jobs are interesting and record their `file-link`.
1. [Get an extract file or files](#get-file)
