...
|`isDeleted`|`boolean`|`true`/`false`|If `true`, the list item is deleted from all lists. The `lists` field includes all lists that contain the list item. If `false`, the list item is not deleted from one or more lists. The `lists` field includes only the lists that contain the list item in not-deleted state.|
