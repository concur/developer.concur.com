...
This API supports both company level and user level access tokens. The user access token is authenticated on scope and user role, the user must be an Expense, Invoice, Shared or Request Configuration Administrator in order to perform POST, PUT, and DELETE actions.
