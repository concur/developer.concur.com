...
* Subscribers interested in the report stops in the legacy external validation step events should filter by the `stepCode` attribute in the payload with a value of `EXTVAL`.
* Subscribers interested in the report is paid events should filter by the `currentPaymentStatus` attribute in the payload with a value of `P_PAID`.
