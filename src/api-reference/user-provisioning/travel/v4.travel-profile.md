...
This endpoint is available to SAP Concur clients and partners who utilize [User Provisioning v4](/api-reference/user-provisioning/v4.user-provisioning.html) to provision or modify users in the Concur Travel extension. It allows callers to retrieve values for the Concur Travel user that have been provisioned via User Provisioning. For partners or TMCs who need to retrieve additional Travel Profile data that is not supported here, these data are available via the [Travel Profile v2 API](/api-reference/travel-profile/v2.profile-service.html).
