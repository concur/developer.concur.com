---
title: User Provisioning Travel Extension v4
layout: reference
---

{% include prerelease.html %}

This endpoint is available to SAP Concur clients and partners who utilize User Provisoning to provision or modify users in the Travel extension. It allow callers to retrieve values for the Travel user that have been provisioned via User Provisioning. For partners (or TMCs) who need to retrieve additional Travel Profile data that is not supported here, these data are available via the Travel Profile 2.0 API. <link>.
...
Name|Description|Endpoint|Fields
---|---|---|---|
 `travel.user.general.read`|Reads general data.|GET|ruleClass, orgUnit, groups, manager,  customFields|
 `travel.user.private.read`|Reads private data.|GET|travelCrsName, xmlProfileSyncId, travelNameRemark|

## <a name="dependencies"></a>Dependencies ##

SAP Concur clients must use the User Provisioning API to provision users in order to use this endpoint to retrieve user data.
...
### <a name="schema-one"></a>Travel Extension
