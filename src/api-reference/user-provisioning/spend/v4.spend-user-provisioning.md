...
Retrieve all the users in the company that have their `country` set to `US`, starting from the first user, with 100 users on the page:

```shell
GET http://localhost:8061/spend/v4/Users?startIndex=1&itemsPerPage=100&filter=urn%3Aietf%3Aparams%3Ascim%3Aschemas%3Aextension%3Aspend%3A2.0%3AUser%3Acountry%20eq%20%22US%22
```
...
Retrieve all the users in the company that have their `custom2` field set to `US`, starting from the 5th user, with 10 users on the page:

```shell
GET http://localhost:8061/spend/v4/Users?startIndex=5&itemsPerPage=10&filter=urn%3Aietf%3Aparams%3Ascim%3Aschemas%3Aextension%3Aspend%3A2.0%3AUser%3AcustomData%5Bid%20eq%20%22custom2%22and%20value%20eq%20%22US%22%5D
```

- startIndex=5
- itemsPerPage=10
- filter=urn:ietf:params:scim:schemas:extension:spend:2.0:User:customData[id eq \\"custom2\\" and value eq \\"US\\"]
