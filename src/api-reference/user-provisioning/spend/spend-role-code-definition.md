...
|N/A|Account Code Administrator|The user assigned this role can view and modify account codes. **Note:** This field is not supported.|N|N|
...
|EXP_CONFIG_ADMIN_CLIENT|Client Expense Administrator|**This custom role is not available to all clients; availability is based on the client configuration.** It combines permissions from the Expense Configuration Administrator role and the Shared Configuration Administrator. The user assigned this role can access these options (some may allow add/edit; some may be view-only): Accounting administration, audit rules, car configuration, email reminders, exceptions, expense Types, locations, policies, travel allowance, workflows|Y|N|
...
|N/A|Concur Mobile User|**Do not use; is not associated with the SAP Concur mobile app. **Note:** This field is not supported.|N|N/A|
...
|N/A|Digital Compliance Administrator|The user assigned this role views the E-Bunsho Timestamp validation tool. **Note:** This field is not supported.|N|N|
...
|N/A|Employee Maintenance|The user assigned this role can manage employees, including assigning roles, delegates, and preferences. However, they cannot create a new user account. **Note:** This field is not supported.|Y|Y|
...
|EXP_CONFIG_ADMIN|Expense Configuration Administrator|**This role is intended to be assigned to and used by SAP Concur internal staff only.**<br>The user assigned this role can fully manage (add, edit, delete) expense group configurations, policies, expense-based forms, fields, validations, and vendor lists, expense report and authorized approver workflows, audit rules, expense types and expense categories, payment types, account codes, exceptions, car configuration and reimbursement, receipt handling including payment hold, scan configurations, receipt limits, and receipt imaging, email reminders, reimbursement currencies, offline settings, configuration change log (view only), taxability and deductibility calculation service|Y|N|
...
|EXP_PROXY_USER|Expense Proxy Logon| he user assigned this role can log on to Expense and act as a proxy user for other employees within an assigned group.|Y|N|
...
|N/A|Expense Type Administrator|The user assigned this role can create, edit, and delete Expense Types. **Note:** This field is not supported.|N|N|
...
|N/A|Expense Processor(Concur Audit)|The user assigned this role can audit reports from within Concur Audit Service. **Note:** This field is not supported.|N|N|
...
|SHD_IMPORT_EXTRACT_ADMIN|Import/Extract Administrator (formerly Integration Administrator)|**This role is intended to be assigned to and used by SAP Concur internal staff only.**<br>A user assigned this role can manage (add, edit, delete) import, extract, archive, and reporting consolidation jobs, schedule jobs, and view the system log.|N|Y|
...
|N/A|Role Builder|**This role is intended to be assigned to and used by SAP Concur internal staff only.** **NOTE:** This field is not supported.|N|Y|
|SHD_CONFIG_ADMIN|Shared Configuration Administrator|**This role is intended to be assigned to and used by SAP Concur internal staff only.** <br>The user assigned this role can fully manage (add, edit, delete) feature hierarchies for Expense Reports, and Vendor Invoices, employee group configurations including assignment of the employee form, employee forms, fields, validations, and custom and connected lists, expense and payment delegate configurations, reimbursement currencies, imaging settings, accounting structure (ledgers), localization tasks, configuration change log (view only).|Y|Y|
...
|EXP_TRAVEL_AND_EXPENSE_USER|Travel and Expense User|The user assigned this role can access SAP Concur. The user requires an additional role (Expense User, Travel User, etc.) to access SAP Concur products. **Note:** This field is not supported.|N|Y|
|N/A|UI Preview|The user assigned this role can preview the enhanced user interface. **Note:** This field is not supported.|N|Y|
...
|INV_CONFIG_ADMIN|Invoice(Payment) Configuration Administrator|**This role is intended to be assigned to and used by SAP Concur internal staff only**<br>The user assigned this role can fully manage (add, edit, delete) Invoice group configurations, policies, Invoice-based forms, fields, and validations, Invoice and authorized approver workflows, audit rules, expense types, account codes and account code hierarchy, exceptions, image handling, including scan configurations, invoice imaging, vendor imaging, email reminders, company info, and configuration change log (view only)|Y|N|
...
|N/A|Invoice Full User|The user assigned this role can access Invoice Processing only. Does not have access to Concur Vendor Payment Classic. **NOTE:** This field is not supported.|N|N|
|N/A|Invoice Pilot User|The user assigned this role can access both Invoice Processing and Concur Vendor Payment Classic. **Note:** This field is not supported.|N|N|
