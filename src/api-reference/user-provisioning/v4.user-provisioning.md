...
* `travel.user.general.read`|general data including RuleClass, Groups, OrgUnit, Travel Custom Fields, Travel Manager| /Users
* `travel.user.private.read`|private data including XML Sync ID, GDS Sync ID (CRS Name), Travel Name Remark| /Users
...
### Response Example
> An limited example of a schema request response.
...
## <a name="Retrieve Travel User Data using GET"></a>Retrieve Travel User Data using GET 

Retrieves Concur Travel user information.  Please see below for further schema information
...
Name|Type|Format|Description
---|---|---|---
`id`|`string`|`UUID`|Concur UUID of the user. 
...
`xmlProfileSyncId`|`string`|-|User-assigned Travel user identifier that allows the user profile to be synchronized with other vendors. IMPORTANT: The following characters cannot be used as a value for this record: "% # ! * & ( ) ~ ` ' { - ^ } \ | / ? > < , ; : " + = []"
...
`orgUnit`|`string`|-|Org Unit for the user. Value must exactly match an Org Unit/Division value setup for the company!
`groups`|`integer`|-|List of user groups that user belongs to for assigning Travel Roles (permissions.)
