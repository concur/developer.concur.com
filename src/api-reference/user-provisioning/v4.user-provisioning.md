...
`identity.user.ids.read`|Read user ID data|/profile/identity/v4/users/
`identity.user.core.read`|Read user core data|/profile/identity/v4/users/
`identity.user.coresensitive.read`|Read core sensitive data|/profile/identity/v4/users/
`identity.user.enterprise.read`|Read user enterprise data|/profile/identity/v4/users/
`travel.user.general.read`|Retrieve travel user general data: RuleClass, Groups, OrgUnit, Travel Custom Fields|/travel/v4/users/
`travel.user.private.read`|Retrieve travel user private data: Sync IDs, Travel Name Remark|/travel/v4/users/
`spend.user.general.writeonly`|Write access to all spend related extentions|/Bulk & /Users
`spend.user.general.read`|To retrieve spend profile information|/spend/v4/users/
...
#### PUT Payload Example
* Concur UUID must be in the path of the request. 
```shell
...
To retrieve profile data for a specific user, Concur has has enabled seperate endpoints for the retrieval of profile data. 
...
Retrieves a detailed provisioning request status by operation status.
...
### Response Example
* "id": "f017648e-9894-46ce-a155-8357e8c97da3", = the SAP Concur user UUID. 
