...
Users can be created via POST with idenitity, spend and travel attritubes within the same request or via a POST to create the identity first and PATCH spend and travel profile attritubes. 
...
```shell
{
    ...
    "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User": {
        "employeeNumber": "John4_29_4@sap.com",
...
Creates one or more provisioning requests via the /Bulk endpoint. Recommended for clients who are performing bulk updates to multile users at the same time. 
...
Updates one or more provisioning requests. Using PATCH is recommended for updating single or mutliple data attritubes within a request. PATCH will only update the data attirutbes listed within the request. 
* Limited to 100 operations or record size of 400KB per request.  
PATCH operations supported:
* add
* remove
...
Using PUT is recommended for updating all data attritubes for a user. PUT will replace data attributes for the user listed within the request and when attributes are not provisioned, system default values will be provisioned. 
* Limited to 100 operations or record size of 400KB per request.  
