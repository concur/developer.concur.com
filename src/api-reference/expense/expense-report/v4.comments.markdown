...
The Comments v4 API can be used to read the comments entered on the expense report header or on the expenses of an existing expense report. This API can be used to infer information about spend that occurs out of policy, which is justified by the report owner or inaccuracies pointed out by the approvers of the expense report.
...
The Reports V2 API provides only the last comment on the report. The documentation can be found [here](./v2.reports.html) 
...
`creationDate`|`string`|YYYY-MM-DDTHH:mm:ssZ.SSS'Z'|**Required**The UTC datetime when the comment was created on the report/expense (https://en.wikipedia.org/wiki/ISO_8601).
`isLatest`|`boolean`|-|**Required**This attribute represents whether a specific comment is the latest comment by the user.
