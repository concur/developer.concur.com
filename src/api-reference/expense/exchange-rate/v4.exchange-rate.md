...
* [Currency Admin Configuration](#currency-admin-configuration)
...
1. Use Currently Loaded Exchange Rates - Ensure this is checked to consume exchange rates. 
1. The Exchange Rate Source - provides three options in which what rates are retrieved:
   1. _Search Local then External Sources_ - Custom rates are loaded first, and, if not found, consume rates from an external source.
   1. _Use Local Exchange Rates Only_ - Custom rates only
   3. _Use External Exchange Rates only_ - Our default external exchange rate source, OANDA. 
1. Exchange rate Markup (percentage) - Increase all rates returned by an additional percentage.
1. Allow Inversion of currency rate pairs - As currency rates are bidirectional, this allows retrieval of a rate in the opposite direction of an uploaded exchange rate, calculated as an inverse of the original rate.
