...
`SpendCategoryCode`	|	`string`	|		|	The spend category code for the quick expense. The available spend category codes are consistent across all SAP Concur products. The values are used in SAP Concur reporting. Format: One of the Code values in the [Spend Category Code List](/api-reference/spend-category-codes.html). Note: COCRM (Company Car Mileage) and PRCAR (Personal Car Mileage) are not currently supported.
