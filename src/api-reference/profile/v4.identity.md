...
`identity.user.coreenterprise.writeonly`|Write only access to all core and enterprise fields except `externalID`.|PATCH, PUT, POST
`identity.user.externalID.writeonly`|Write only access to `externalID`.|PATCH, PUT, POST
