...
`identity.user.externalID.writeonly`|Write acess to `externalID` only.|PUT, POST, PATCH
...
`name`|`string`|-|**Required** Name of the emergency contact name.
...
`relationship`|`string`|-|**Required** Emergency contact relationship to the user.
`streetAddress`|`string`|-|The full street address component, which may include house number, street name, P.O. box, and multi-line extended street address information.
...
`preferenceCurrencySymbolLocation`|`string`|-|Preferred currency symbol location for the user.
`preferenceDateFormat`|`string`|-|Preferred date format for the user.
`preferenceDefaultCalView`|`string`|-|Preferred default calendar view for the user.
`preferenceDistance`|`string`|-|Preferred distance metric.
...
`userName`|`string`|-|**Required** The name that can be used to login to Concur Travel and Expense.
...
`entitlements`|`string`|-|The features enabled for the company.
...
`loginViaSsoOnly`|`boolean`|-|If `true`, the login is available via SSO only.
`passwordPolicy`|`object`|-|-
`daysUntilExpiration`|`integer`|-|Number of days until password expiration.
`expirePasswordOnUserCreation`|`boolean`|-|If `true`, the password will expire on user creation.
...
`mobileRequiresMixedCase`|`boolean`|-|If `true`, the password will require mixed cases for mobile.
`mobileRequiresNonalphanum`|`boolean`|-|If `true`, the password will require non-alphanumeric characters for mobile.
...
`passwordResetEmailPolicy`|`string`|-|When the password reset email should be sent.
`passwordResetSupportEmail`|`string`|-|The from address of the password or PIN reset email.
`requiresMixedCase`|`boolean`|-|If `true`, the password will require mixed cases.
`requiresNonAlpha`|`boolean`|-|If `true`, the password will requires non-alphabetic characters.
`requiresNonAlphanum`|`boolean`|-|If `true`, the password will require non-alphanumeric characters.
`requiresNumber`|`boolean`|-|If `true`, the password will require numbers.
`requiresSecurityQuestions`|`boolean`|-|If `true` , security questions will be required.
`restrictPasswordResetOncePerDay`|`boolean`|-|If `true`, password resets will be limited to once a day.
`channels`|`string`|-|-
`enabled`|`boolean`|-|If `true`, channels will be enabled.
...
`type`|`string`|-|**Required** The error message type.
