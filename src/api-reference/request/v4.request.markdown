...
value|string|-|The value of the non-list item. For list fields, this value will be blank. <br>In case of an UPDATE of the Custom field, the value must either contain the text data for a non-list item or the unique id of the item (listItemId value) for a list item
