...
> **Limitations**: This API is only available to clients who have been granted access by SAP Concur. Access to this documentation does not provide access to the API. This API is not available in Implementation environments.
...
Name|Type|Format|Description
---|---|---|---
`accountCode`|`string`|-|The account code, if not provided, will be read from the employee profile configuration data.
...
`purpose`|`string`|-|Purpose of raising a cash advance.
...
`exchangeRate`|-|[`exchangeRate`](#exchange-rate-schema)|Details on the exchange rate.
...
`exchangeRate`|`number`|-|The exchange rate that applies to the expected expense Default: `1`
