...
[FI Documents](#schema-PagedResources.Resource.FIDocument)
...
Name|Type|Format|Description
---|---|---|---
`docType`|`string`|`path`|**Required** The financial document type to return. `expense`, `invoice`, `cashadvance`, `payroll`, `obligation`. Only one type of transactions can be retrieved at a time
