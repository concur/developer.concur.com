...
SAP Concur will be deprecating Bulk User 3.1 for the US and EMEA data centers on or before June 30th, 2021. This API will be replaced by Identity v4. After the deprecation of Bulk User v3.1 decommission will follow.
...
SAP Concur has modified the Travel Profile 2.0 API `CellPhoneNumber` field to accept parenthesis in addition to dashes, spaces, and digits. Previously, parenthesis were not allowed and resulted in an error.
...
This change was made to make the Travel Profile 2.0 API consistent with other SAP Concur APIs that accept or return the user’s cell (sometimes called mobile) number.
